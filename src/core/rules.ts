...
function hasValue(args: readonly string[], predicate: (value: string) => boolean): boolean {
...
function evidence(command: ShellCommand): string {
  return command.command.length > 240 ? `${command.command.slice(0, 237)}...` : command.command;
}

function finding(command: ShellCommand, spec: FindingSpec): RuleFinding {
  return {
    ...spec,
...
function broadPath(path: string, context: EvaluationContext): boolean {
  const normalized = path.replace(/^['"]|['"]$/g, "");
  if (SAFE_TEMP_PREFIXES.some((prefix) => normalized.startsWith(prefix))) return false;
  if (["/", "/*", ".", "./", "..", "../", "~", "~/*", "$HOME", "${HOME}"].includes(normalized)) return true;
  if (context.cwd && (normalized === context.cwd || normalized === `${context.cwd}/`)) return true;
  return false;
}

function allSafeTemp(paths: readonly string[]): boolean {
...
function gitArgs(args: readonly string[]): string[] {
  ...
  while (result[0]?.startsWith("-")) {
    ...
    if (["-C", "-c", "--git-dir", "--work-tree", "--namespace"].includes(option)) result.shift();
  }
  return result;
}

function sqlText(command: ShellCommand): string | undefined {
  const exe = basename(command.executable);
  if (!(exe === "psql" || exe === "mysql" || exe === "mariadb" || exe === "sqlite3" || exe === "duckdb")) return undefined;
...
function evaluateFilesystem(command: ShellCommand, context: EvaluationContext): RuleFinding[] {
  const exe = basename(command.executable);
  const args = command.argv.slice(1);
...
function evaluateGit(command: ShellCommand): RuleFinding[] {
  if (!["git", "git.exe"].includes(basename(command.executable))) return [];
...
function evaluateSystem(command: ShellCommand): RuleFinding[] {
  const exe = basename(command.executable);
  const args = command.argv.slice(1);
...
function evaluatePermissions(command: ShellCommand): RuleFinding[] {
  const exe = basename(command.executable);
  const args = command.argv.slice(1);
...
function evaluateDatabase(command: ShellCommand): RuleFinding[] {
...
function evaluatePackageManager(command: ShellCommand): RuleFinding[] {
  const exe = basename(command.executable);
  const args = command.argv.slice(1);
...
function evaluateContainers(command: ShellCommand): RuleFinding[] {
  const exe = basename(command.executable);
  const args = command.argv.slice(1);
...
function evaluateCloudRemote(command: ShellCommand): RuleFinding[] {
  const exe = basename(command.executable);
  const args = command.argv.slice(1);
...
function synthetic(command: ShellCommand, executable: string, argv: string[], origin: ShellCommand["origin"] = "wrapper"): ShellCommand {
  return { ...command, command: argv.join(" "), executable, argv, origin };
}

function evaluateWrapper(command: ShellCommand, context: EvaluationContext): RuleFinding[] {
  const exe = basename(command.executable);
  const args = command.argv.slice(1);
  if (!EXEC_WRAPPERS.has(exe)) return [];
  ...
  if (exe === "find" && (args.includes("-delete") || args.includes("-exec") || args.includes("-execdir"))) {
    ...
    if (args.includes("-delete")) return [finding(command, {
  ...
  while (index < args.length) {
    ...
    if (exe === "timeout" && (/^\d/.test(value) || value.endsWith("s") || value.endsWith("m"))) { index += 1; continue; }
    if (exe === "nice" && (value === "-n" || value === "--adjustment")) { index += 2; continue; }
    if (value.startsWith("-") || (exe === "env" && (/^[A-Za-z_][A-Za-z0-9_]*=/.test(value) || value === "-i"))) { index += 1; continue; }
    break;
  }
  ...
  if (SHELLS.has(basename(target))) {
    ...
    if (script) return extractShellCommands(script).flatMap((nested) => evaluateCommandRules(nested, context));
  }
  return evaluateCommandRules(inner, context);
}

function evaluateInlineInterpreter(command: ShellCommand): RuleFinding[] {
  const exe = basename(command.executable);
  const args = command.argv.slice(1);
...
export function evaluateCommandRules(command: ShellCommand, context: EvaluationContext): RuleFinding[] {
  const findings = [
