...
import { evaluateCommandRules } from "./rules.js";
...
function allowedByPattern(command, patterns) {
...
function sourceFinding(command, ruleId, category, message, remediation) {
    return {
        ruleId,
        category,
        severity: "critical",
...
function sourceLevelFindings(command) {
    const findings = [];
...
export function evaluateCommand(command, options = {}) {
    ...
    for (const shellCommand of parsed) {
        if (allowedByPattern(shellCommand, options.allow ?? []))
            continue;
        for (const finding of evaluateCommandRules(shellCommand, context)) {
            if (categoryEnabled(finding.category, options.categories))
