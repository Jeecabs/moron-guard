...
function validCategories(values) {
    if (!Array.isArray(values))
        return undefined;
    const categories = values.filter((value) => typeof value === "string" && CATEGORIES.includes(value));
    return categories.length > 0 ? categories : undefined;
}
function readJson(path) {
    try {
        if (!existsSync(path) || readFileSync(path, "utf8").length > 256 * 1024)
            return undefined;
...
function boolEnv(value) {
    if (value === undefined)
        return undefined;
    if (["1", "true", "on", "yes"].includes(value.trim().toLowerCase()))
        return true;
    if (["0", "false", "off", "no"].includes(value.trim().toLowerCase()))
        return false;
    return undefined;
}
function modeEnv(value) {
...
function restrictProjectConfig(config, warnings) {
    const safeValues = [
        ["enabled", true],
        ["mode", "enforce"],
        ["failClosed", true],
    ...
    const sensitiveKeys = ["categories", "allow", "maxDepth", "maxCommandBytes"];
    ...
    const restricted = Object.fromEntries(accepted);
    if (ignored.length > 0) {
...
export function loadMoronConfig(cwd, env = process.env) {
    const warnings = [];
    const sources = [];
    ...
    for (const source of paths) {
        if (!existsSync(source.path)) {
            if (explicit)
                warnings.push(`Could not read MORON_GUARD_CONFIG: ${source.path}`);
            continue;
        }
    ...
    const allow = env.MORON_GUARD_ALLOW
        ...
        : Array.isArray(merged.allow) ? merged.allow.filter((item) => typeof item === "string" && item.trim().length > 0) : undefined;
    ...
    for (const name of ["MORON_GUARD_BIN", "DCG_BIN", "MORON_GUARD_TIMEOUT_MS"]) {
        if (env[name])
            warnings.push(`${name} is ignored; Moron Guard is in-process.`);
    }
    for (const name of ["MORON_GUARD_ENABLED", "MORON_GUARD_MODE", "MORON_GUARD_FAIL_CLOSED", "MORON_GUARD_USER_BASH", "MORON_GUARD_ALLOW_PROJECT_CONFIG"]) {
        if (env[name] !== undefined && ((name === "MORON_GUARD_MODE" && !envMode) || (name !== "MORON_GUARD_MODE" && boolEnv(env[name]) === undefined)))
            warnings.push(`Invalid ${name}; safe defaults retained.`);
    }
    return {
        path: selectedPath,
        sources,
